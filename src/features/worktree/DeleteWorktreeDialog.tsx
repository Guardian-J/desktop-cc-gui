...
export function DeleteWorktreeDialog({
    ...
    void (async () => {
      ...
      const entry =
        listRes.status === "fulfilled" && listRes.value
          ? listRes.value.find((w) => w.path === workspace.path)
          : undefined;
      setPreflight({
  ...
  }, [workspace.path, repoPath, branch, baseRef]);
    ...
    >
      <div className="flex flex-col gap-3">
        ...
        <div className="mt-1 flex justify-end gap-2 border-t border-separator-border pt-3">
          ...
          >
            {!repoPath
              ? t("worktree.deleteUnregisterSubmit")
              : deleteBranch && branch
                ? t("worktree.deleteSubmitWithBranch")
                : t("worktree.deleteSubmit")}
          </Button>
        </div>
      </div>
    </ModalShell>
